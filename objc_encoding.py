...
_C_ID = '@' # Object pointer, optionally followed by a quoted class name (if no class name is present, represents id)
_C_CLASS = '#' # Class
_C_SEL = ':' # Selector
...
# Array brackets, containing the number of array elements, followed by the encoding for the element type
_C_ARY_B = '['
_C_ARY_E = ']'
# Union parentheses, containing the unqoted union name (? if unknown), optionally followed by an = and any number of encodings for the field type, each optionally preceded by a quoted field name
_C_UNION_B = '('
_C_UNION_E = ')'
# Struct braces, containing the unqoted struct name (? if unknown), optionally followed by an = and any number of encodings for the field type, each optionally preceded by a quoted field name
_C_STRUCT_B = '{'
_C_STRUCT_E = '}'
##_C_VECTOR = '!' # Vector? Not listed in the Objective-C Runtime Programming Guide
_C_CONST = 'r' # const qualifier

# "Unofficial" constant names for encodings listed in the Objective-C Runtime Programming Guide, but not in objc/runtime.h.
...
class UnknownType(BaseType):
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}()".format(cls=type(self), self=self)
...
class Scalar(BaseType):
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}({self.type!r})".format(cls=type(self), self=self)
...
class BitField(InternalType):
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}({self.width!r})".format(cls=type(self), self=self)
...
class Pointer(BaseType):
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}({self.element_type!r})".format(cls=type(self), self=self)

    def encode(self):
        if isinstance(self.element_type, Scalar) and self.element_type.type == "char":
            return _C_CHARPTR
        else:
...
class ID(BaseType):
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}({self.class_name!r})".format(cls=type(self), self=self)
...
class Array(BaseType):
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}({self.length!r}, {self.element_type!r})".format(cls=type(self), self=self)

    def encode(self):
        return "{}{}{}{}".format(_C_ARY_B, self.length, self.element_type.encode(), _C_ARY_E)
...
class Field(InternalType):
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}({self.name!r}, {self.type!r})".format(cls=type(self), self=self)
...
class Struct(BaseType):
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}({self.name!r}, {self.fields!r})".format(cls=type(self), self=self)
    ...
    def encode(self):
        ...
        if self.fields is None:
            ...
            return "{}{}={}{}".format(_C_STRUCT_B, name, "".join(field.encode() for field in self.fields), _C_STRUCT_E)
...
class Union(BaseType):
    ...
    def encode(self):
        ...
        if self.fields is None:
            ...
            return "{}{}={}{}".format(_C_UNION_B, name, "".join(field.encode() for field in self.fields), _C_UNION_E)

class QualifiedType(BaseType):
    __slots__ = ("type", "const", "in_", "inout", "out", "bycopy", "byref", "oneway")

    def __init__(self, type, *, const=False, in_=False, inout=False, out=False, bycopy=False, byref=False, oneway=False):
        super().__init__()
        self.type = type
        self.const = const
        self.in_ = in_
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}(type={self.type!r}, const={self.const!r}, in_={self.in_!r}, inout={self.inout!r}, out={self.out!r}, bycopy={self.bycopy!r}, byref={self.byref!r}, oneway={self.oneway!r})".format(cls=type(self), self=self)
...
class Property(object):
    ...
    def __repr__(self):
        return "{cls.__module__}.{cls.__qualname__}(type={self.type!r}, name={self.name!r}, readonly={self.readonly!r}, copy={self.copy!r}, retain={self.retain!r}, nonatomic={self.nonatomic!r}, getter={self.getter!r}, setter={self.setter!r}, dynamic={self.dynamic!r}, weak={self.weak!r}, gc={self.gc!r}, old_encoding={self.old_encoding!r})".format(cls=type(self), self=self)
...
def _decode_internal(enc, pos):
    """Decode the start of the given type encoding and return a tuple (tp, used) containinc the decoded type and the number of characters "consumed". Raise a ValueError if the encoding is not valid.

    If enc is a substring of a longer type encoding, pos should be enc's position in the source string, and 0 otherwise. (This value is only used in error messages.)
    """
    ...
    try:
        ...
        if enc[0] == _C_PTR:
            ...
            if len(enc) > 1 and enc[1] == '"':
                ...
                if not sep:
                    raise ValueError("EOL while decoding object class name (position {})".format(pos))
            ...
            if i == 1:
                raise ValueError("EOL while decoding bit field (position {})".format(pos))
            ...
            if i == 1:
                raise ValueError("Missing length in array (position {})".format(pos))

            tp, used = _decode_internal(enc[i:], pos+i)

            if enc[i+used] != _C_ARY_E:
                raise ValueError("Expected end of array (position {})".format(pos+i+used))
            ...
            if end_pos == -1:
                raise ValueError("EOL while decoding struct or union name (position {})".format(pos+1))
            else:
                if eq_pos == -1:
                    name_end = end_pos
                else:
            ...
            while enc[i] != end:
                ...
                if enc[i] == '"':
                    ...
                    if end_quote_pos == -1:
                        raise ValueError("EOL while decoding struct or union field name (position {})".format(pos+i))
            ...
            raise ValueError("Invalid start of encoding: {!r} (position {})".format(enc[0], pos))

def _decode_qualified_internal(enc, pos):
    """Decode the start of enc, like _decode_internal, except that type qualifiers (as found in some method signatures) are recognized. The result is returned as a QualifiedType."""
    ...
    for i, char in enumerate(enc):
        if char in seen_qualifiers:
            raise ValueError("Qualifier {} appeared more than once (position {})".format(pos+i))
...
@functools.lru_cache()
def decode(enc):
    """Decode the given type encoding into an instance of the appropriate BaseType subclass.

    If the encoding is empty, invalid or contains multiple types, raise a ValueError.
    """

    tp, used = _decode_internal(enc, 0)
    if used < len(enc):
        raise ValueError("Encoding contains more than one type ({} characters remaining)".format(len(enc) - used))
    return tp

@functools.lru_cache()
def decode_method_signature(enc):
    """Decode the given method signature (i. e. the type encodings for the return type and all argument types concatenated) into a tuple (restype, argtypes).

    restype is the method return type (the first type encoding) as a QualifiedType.
    argtypes is a list of all argument types (all remaining type encodings, if any), each as a QualifiedType.
...
def decode_property(enc):
    ...
    If the encoding is missing the T or V part or is otherwise invalid, raise a ValueError.
    """
    ...
    for i, part in enumerate(parts):
        ...
        if part_id in seen_parts:
            raise ValueError("Part {} (index {}) is present more than once".format(part[0], i))
        ...
        if part_id == "T":
            ...
            if i != len(parts)-1:
                raise ValueError("Part V must be at the last index ({}), not {}".format(len(parts)-1, i))
            ...
            raise ValueError("Unknown part {} (index {})".format(part_id, index))
            ...
            for i, field in enumerate(tp.fields):
                ...
                if isinstance(field.type, BitField):
                    if field.type.width == 1:
                        ...
                        field_decl = (ctypes.c_uint, field_name, field.type.width)
                else:
                    field_decl = (field_name, _encoding_type_to_ctypes(field.type))
...
def main():
    restype, argtypes = decode_method('{CGRect={CGPoint=dd}{CGSize=dd}}@:{CGRect={CGPoint=dd}{CGSize=dd}}{CGPoint=dd}')
